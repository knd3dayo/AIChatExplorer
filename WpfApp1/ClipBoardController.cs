...
namespace WpfApp1
{
    public class ClipboardController
    {
        ...
        private static void processClipboardItem(SharpClipboard.ContentTypes contentTypes, string content, ClipboardChangedEventArgs e)
        {
            ...
            {
                // 重複したアイテムを削除
                RootFolder.RemoveDuplicateItems(item);
                // アイテムを追加
                RootFolder.UpsertItem(item);
            }
        ...
        public static ObservableCollection<ClipboardItem> GetClipboardItems(ClipboardItemFolder folder)
        {
            ...
            // folderのAbsoluteCollectionNameを取得
            var collectionName = folder.AbsoluteCollectionName;
            // folderのItemsを取得
            var itemCollection = ClipboardDatabaseController.GetClipboardDatabase().GetCollection<ClipboardItem>(collectionName);
            var item = itemCollection.FindAll();
