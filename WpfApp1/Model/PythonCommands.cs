...
namespace WpfApp1.Model
{
    public class PythonCommands
    {
        ...
        public static void ExtractTextCommandExecute(object obj)
        {
            ...
            try
            {
                string text = PythonExecutor.ExtractText(MainWindowViewModel.Instance.SelectedItem.ClipboardItem.Content);
                System.Windows.Clipboard.SetText(text);
                MainWindowViewModel.Instance?.UpdateStatusText(MainWindowViewModel.Instance.SelectedItem.ClipboardItem.Content + "から抽出したテキストをクリップボードに貼り付けました");
        ...
        public static void MaskDataCommandExecute(object obj)
        {
            ...
            try
            {
                string maskedText = PythonExecutor.MaskData(MainWindowViewModel.Instance.SelectedItem.ClipboardItem.Content);
        ...
        public static void RunPythonScriptCommandExecute(object obj)
        {
            ...
            try
            {
                ClipboardItem? resultItem = PythonExecutor.RunScript(scriptItem, Instance.SelectedItem.ClipboardItem);
                MainWindowViewModel.Instance?.UpdateStatusText("Pythonスクリプトを実行しました");
                if (resultItem != null)
                {
                    // 実行結果をクリップボードにコピー
        ...
        public static void OpenAIChatCommandExecute(object obj)
        {
            ...
            if (Instance.SelectedItem != null)
            {
                openAIChatWindowViewModel.InpupText = Instance.SelectedItem.ClipboardItem.Content;
            }
