...
from autogen_group_chat_debug_02 import create_model_client, create_agent, create_termination_condition
...
async def execute_agent(
    ...
    agent = [item for item in autogen_agents if item.name == agent_name]
    agent = agent[0] if len(agent) > 0 else None
...
async def main(message: str):
    ...
    agent_caller = create_agent(
        ...
        model_client=model_client,
        tools=[
            FunctionTool(execute_agent, execute_agent.__doc__, name = "execute_agent"), 
            FunctionTool(list_agents, list_agents.__doc__ ,name = "list_agents")
        ]
    )
    # plannerエージェント
    ...
    # グループチャットを実行
    stream = chat.run_stream(task=message)
