...
import clipboard_app.ai_app_wrapper
import clipboard_app.ai_app
...
async def main():
    ...
    if props_file:
        ...
        with open(props_file, "r", encoding="utf-8") as f:
            ...
            # autogen_props 
            autogen_props = clipboard_app.openai_modules_app_wrapper.get_autogen_objects(props_dict)
    ...
    # AutogenGroupChatを実行
    message_count = 0
    async for message in clipboard_app.openai_modules_app.run_autogen_chat(autogen_props,  input_text):
        if not message:
            break
        message_count += 1
