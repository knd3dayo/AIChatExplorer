...
async def openai_chat_async(request_json: str) -> dict:
    ...
    result: dict = await ai_app.run_openai_chat_async(openai_props, vector_db_items, chat_request_context, chat_request_dict)
    return result
...
async def extract_text_from_file_async(request_json: str) -> dict:
    ...
    text: str = await ai_app.extract_text_from_file_async(filename)
    return {"output": text}
...
async def extract_base64_to_text_async(request_json: str):
    ...
    text: str = await ai_app.extract_base64_to_text_async(base64_data, extension)
    return {"output": text}
